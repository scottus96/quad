...
//THESE WILL BE DIFFERENT FOR YOUR WIRING, change the value as needed
const int left = A3;
const int bottom = A5;
const int top = A2;
const int right = A4;
const int button_fsr = A1; // ignore if no FSR is wired as a action-button
...
void loop() {
    ...
    //Validation of wiring connections, uncomment and run to test sensors are connected and functioning
    //should output between 0-1024 for lengths and ~40-180 for angles (could require some mapping adjustment)
   /*
   Serial.println("Length order: Top, Right, Bottom, Left:");
   Serial.println(analogRead(top));
   Serial.println(analogRead(right));
   Serial.println(analogRead(bottom));
   ...
   square();
   Serial.println("Angle order: bottom right, top left, bottom left, top right:");
...
*/
  ...
  if (Serial.available() > 0) {
    top_len = mmap(analogRead(top),149,1015,5.77,9.32);
    right_len = mmap(analogRead(right),124,1016,5.77,9.22);
    bottom_len = mmap(analogRead(bottom),102,1015,5.77,9.21);
    left_len = mmap(analogRead(left),116,1003,5.77,9.22);

    inputVal = analogRead(pot_pin);

    //mapping the potentiometer values (0-1024) to angles on RANDALL
    angle1 = (map(inputVal, 942, 526, 176, 90));
    square();
  ...
  if (button > 150) {
    ...
    inputVal = analogRead(pot_pin);
    //mapping the potentiometer values (0-1024) to angles on RANDALL
    angle1 = (map(inputVal, 942, 535, 176, 90));
    square();
